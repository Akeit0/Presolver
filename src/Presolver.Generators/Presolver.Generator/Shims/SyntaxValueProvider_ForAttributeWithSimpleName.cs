...
// Copied from https://github.com/dotnet/runtime/blob/1473deaa50785b956edd7d078e68c0581c1b4d95/src/libraries/Common/src/Roslyn/SyntaxValueProvider_ForAttributeWithSimpleName.cs
...
internal static partial class SyntaxValueProviderExtensions
{
    ...
    /// </summary>
    sealed class SyntaxTreeInfo : IEquatable<SyntaxTreeInfo>
    {
        ...
        {
            return Equals(obj as SyntaxTreeInfo);
        }
    ...
    /// </summary>
    static readonly ConditionalWeakTable<SyntaxTree, SyntaxTreeInfo> s_treeToInfo = new();
    ...
    ///     generics, containing types, or namespaces.  For example <c>CLSCompliantAttribute</c> for
    ///     <see
    ///         cref="System.CLSCompliantAttribute" />
    ///     .
    ///     <para />
    ///     This provider understands <see langword="using" /> (<c>Import</c> in Visual Basic) aliases and will find
    ...
    ///     Then
    ///     <c>
    ///         context.SyntaxProvider.CreateSyntaxProviderForAttribute(nameof(CLSCompliantAttribute), (node, c) => node is
        ...
        Func<SyntaxNode, CancellationToken, bool> predicate)
    {
            ...
            SyntaxNode compilationUnit)
        {
            ...
            CSharpSyntaxHelper.Instance.AddAliases(compilationUnit, ref globalAliases, true);

            return GlobalAliases.Create(globalAliases.AsSpan().ToImmutableArray());
        }
    }

    static SyntaxTreeInfo GetTreeInfo(
    ...
    static ImmutableArray<SyntaxNode> GetMatchingNodes(
        ...
        CancellationToken cancellationToken)
    {
            ...
            ref ValueListBuilder<SyntaxNode> attributeTargets)
        {
            ...
            syntaxHelper.AddAliases(compilationUnit, ref localAliases, false);
            ...
            ref ValueListBuilder<SyntaxNode> attributeTargets)
        {
            ...
            syntaxHelper.AddAliases(namespaceBlock, ref localAliases, false);
                ...
                while (nodeStack.Length > 0)
                {
                    ...
                    if (syntaxHelper.IsAttributeList(node))
                    {
                        foreach (var attribute in syntaxHelper.GetAttributesOfAttributeList(node))
                        {
                            ...
                            if (matchesAttributeName(ref localAliases, ref seenNames, simpleAttributeName, false) ||
                                matchesAttributeName(ref localAliases, ref seenNames, simpleAttributeName, true))
                            {
            ...
            bool withAttributeSuffix)
        {
            ...
            {
                if (matchesName(currentAttributeName, name, false))
                    return true;
            }
            ...
            foreach (var (aliasName, symbolName) in localAliases.AsSpan())
                ...
                if (matchesName(currentAttributeName, aliasName, withAttributeSuffix) &&
                    matchesAttributeName(ref localAliases, ref seenNames, symbolName, false))
                    return true;
    ...
    static bool ContainsAttributeList(SyntaxNode node)
    {
