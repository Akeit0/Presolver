...
// Copied from https://github.com/dotnet/runtime/blob/1473deaa50785b956edd7d078e68c0581c1b4d95/src/libraries/System.Private.CoreLib/src/System/Collections/Generic/ValueListBuilder.cs
...
internal ref struct ValueListBuilder<T>
{
    ...
    {
        if ((uint)(_pos + source.Length) > (uint)_span.Length) Grow(_span.Length - _pos + source.Length);
    ...
    public void Insert(int index, scoped ReadOnlySpan<T> source)
    {
        ...
        if ((uint)(_pos + source.Length) > (uint)_span.Length) Grow(source.Length);
    ...
    void Grow(int additionalCapacityRequired = 1)
    {
        ...
        var nextCapacity = Math.Max(_span.Length != 0 ? _span.Length * 2 : 4, _span.Length + additionalCapacityRequired);
        ...
        if ((uint)nextCapacity > ArrayMaxLength) nextCapacity = Math.Max(Math.Max(_span.Length + 1, ArrayMaxLength), _span.Length);
